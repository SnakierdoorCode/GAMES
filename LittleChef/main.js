...
Promise.all([
    mergeFiles(getParts(rootBuildPath + "Build/LittleChef_Itch.data", 1, 1)),
    mergeFiles(getParts(rootBuildPath + "Build/LittleChef_Itch.wasm", 1, 2))
]).then(([dataBuffer, wasmBuffer]) => {
    window.fetch = async function (url, ...args) {
        let urlStr = typeof url === 'string' ? url : (url && url.url) || '';

        if (urlStr.endsWith("LittleChef_Itch.data") || urlStr.endsWith("build.data")) {
            return new Response(dataBuffer, {
                status: 200,
                statusText: "OK",
                headers: { "Content-Type": "application/octet-stream" }
            });
        } else if (urlStr.endsWith("LittleChef_Itch.wasm") || urlStr.endsWith("build.wasm")) {
