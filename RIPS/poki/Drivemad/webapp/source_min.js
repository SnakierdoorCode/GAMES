...
let g_submitScore = (name, score) => {}
;
function pokiSubmitScore(levelID, score) {
    g_submitScore(levelID, score)
}
function initPokiSdk() {
    ...
    PokiSDK.init({
        submitScore: (submitScoreFromPoki) => {
            g_submitScore = submitScoreFromPoki;
        }
    }).then( () => {
        setPokiInited();
    }
...
function PokiShowLeaderboard(leaderboardID) {
    PokiSDK.showLeaderboard(leaderboardID);
}
...
function pokiSendLevelData() {
    // Sitelock removed - do not send remote level data when running locally.
}
...
function getHostname() {
    ...
    const stringOnWasmHeap = _malloc(1);
    HEAP8[stringOnWasmHeap] = 0;
...
async function performRequest(method, url, body, useToken, parseJson) {
    url = encodeURI(url);
    simpleLogC(`performRequest(${method}, ${url}, ${useToken}, ${parseJson})`);
    let idToken = null;
    if (useToken) {
        throw "error-no-user";
    }
    return new Promise( (resolve, reject) => {
        ...
        xhr.responseType = parseJson ? 'text' : 'blob';
...
function fetchUrl(url, id, useToken) {
    let result = null;
    performRequest("GET", url, null, useToken, false).then( (x) => {
        result = x;
    }
    ).finally( () => {
        if (result === null) {
            Module._web_command_fetch_send_url_done(0, 0, id);
        } else {
            result.arrayBuffer().then( (array) => {
                const bytes = new Uint8Array(array);
                const data_size = bytes.length;
                const data_ptr = _malloc(data_size);
                Module.HEAPU8.set(bytes, data_ptr);
                Module._web_command_fetch_send_url_done(data_ptr, data_size, id);
            }
            );
        }
    }
...
function sendUrl(url, id, dataPtr, dataSize, useToken) {
    const bytes = HEAPU8.subarray(dataPtr, dataPtr + dataSize);
    performRequest("PUT", url, bytes, useToken, false).finally( () => {
        Module._web_command_fetch_send_url_done(0, 0, id);
    }
    );
}
