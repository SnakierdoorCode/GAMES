...
'use strict';
{
    if (window["C3_Is_Supported"]) {
        ...
        window["c3_runtimeInterface"] = new self.RuntimeInterface({
            ...
            projectScripts: [["scripts/project/FrameRateLimit.js"], ["scripts/project/scriptsInEvents.js"]],
...
;'use strict';
{
    ...
    class C3BannerAd {
        constructor(id, size, position, offset, resolver) {
            if (!id)
                return Failure(resolver, "Unit ID not specified");
            if (!size)
                return Failure(resolver, "Ad size not specified");
            if (!position)
        ...
        Release() {
            if (this._remove_load_listeners)
        ...
        Load(adPersonalization, resolver) {
            if (this._ad)
                return Success(resolver, "banner ad loaded");
            ...
            this._load_settled_promise = new Promise(resolve => this._load_settled_promise_resolver = resolve);
            this._ad["load"]()
        }
        async Show(resolver) {
            if (!this._ad)
                return Failure(resolver, "banner ad not created");
            if (this._adLoading)
    ...
    function _AddLoadListeners(resolver) {
        const onLoad = event => {
            ...
            if (this._load_settled_promise_resolver)
...
;'use strict';
{
    ...
    class C3InterstitialAd {
        ...
        ResolveLoadPromise(result) {
            ...
            if (this._loadPromiseResolver)
                this._loadPromiseResolver(result);
            this._loadPromise = null;
            this._loadPromiseResolver = null;
            this._loadPromise = this.GetLoadPromise()
        }
    ...
    function _AddLoadListeners(resolver) {
        const onLoad = event => {
            ...
            if (resolver)
                Success(resolver, "interstitial ad load");
            document.removeEventListener(Events.LOAD, onLoad);
            document.removeEventListener(Events.LOAD_FAIL, onFailLoad)
        }
        ;
    ...
    function _AddShowListeners(resolver) {
        const onShow = event => {
            ...
            if (resolver)
                Success(resolver, "interstitial ad show");
        ...
        const onShowFail = event => {
            this._adShowing = false;
            if (resolver)
                Failure(resolver, "interstitial ad failed to show");
            document.removeEventListener(Events.SHOW, onShow);
            document.removeEventListener(Events.SHOW_FAIL, onShowFail)
        }
        ;
...
;'use strict';
{
    ...
    class C3RewardedAd {
        ...
        async Show(resolver) {
            if (!this._ad)
                return Failure(resolver, "rewarded ad not created");
            if (this._adShowing)
    ...
    function _AddLoadListeners(resolver) {
        const onLoad = event => {
            ...
            this.ResolveLoadPromise(true);
            if (resolver)
                Success(resolver, "rewarded ad load");
            document.removeEventListener(Events.LOAD, onLoad);
            document.removeEventListener(Events.LOAD_FAIL, onFailLoad)
        }
        ;
    ...
    function _AddShowListeners(resolver) {
        ...
        const onShowFail = event => {
            this._adShowing = false;
            if (resolver)
                Failure(resolver, "rewarded ad failed to show");
            document.removeEventListener(Events.SHOW, onShow);
            document.removeEventListener(Events.SHOW_FAIL, onShowFail);
        ...
        const onDismiss = event => {
            if (this._reward !== null) {
                ...
                if (resolver)
                    Success(resolver, JSON.stringify([type, amount]))
            } else if (resolver)
                Failure(resolver, "closed with no reward");
            this._reward = null;
            this._adShowing = false;
            this._adLoading = true;
            this._loadSuccess = false;
...
;'use strict';
{
    ...
    class C3RewardedInterstitialAd {
        ...
        HandleOnLoadedResult(result, resolver) {
            if (result)
                ...
                C3AdUtils.Failure(resolver, "rewarded interstitial ad failed to load")
        }
        async Show(resolver) {
            if (!this._ad)
    ...
    function _AddLoadListeners(resolver) {
        const onLoad = event => {
            ...
            this.ResolveLoadPromise(true);
            if (resolver)
                Success(resolver, "rewarded interstitial ad load");
            document.removeEventListener(Events.LOAD, onLoad);
            document.removeEventListener(Events.LOAD_FAIL, onFailLoad)
        }
        ;
...
;'use strict';
{
    ...
    class C3Consent {
        ...
        async RequestIDFA(resolver) {
            if (ANDROID)
                return Success(resolver, "not-determined");
            if (IOS) {
        ...
        async RequestInfoUpdateForTesting(debug, debugLocation, resolver) {
            const ret = await _RequestInfoUpdateForTesting.call(this, debug, debugLocation, error => {
                if (error)
                    ...
                    Failure(resolver, "requestInfoUpdate error")
            }
            );
            return ret
        }
...
;'use strict';
{
    ...
    addHandler("ShowVideoAdvert", async fn => {
        if (!videoAd)
            return C3AdUtils.Failure(fn, "video ad not created");
        videoAd.Show(fn)
    }
    );
    ...
    addHandler("CreateRewardedInterstitialAdvert", async (id, debug, fn) => {
        if (rewardedInterstitialAd) {
            if (rewardedInterstitialAd.IsShowing())
                return C3AdUtils.Success(fn, "rewarded interstitial ad already showing");
    ...
    addHandler("Configure", async (id, pubID, privacyPolicy, showFree, showConsent, debug, debugLocation, showOnStartUp, fn) => {
        ...
        if (firstCall)
            await C3Consent.Reset();
        if (!await C3Consent.RequestInfoUpdateForTesting(debug, debugLocation, fn))
            return C3Consent.FailureResponse(fn, "failure to update consent information");
        if (firstCall)
            if (showOnStartUp)
    ...
    addHandler("SetMaxAdContentRating", async (label, fn) => {
        if (!await C3AdUtils.StartAdmobPlus(fn))
            return;
        if (!label)
            return C3AdUtils.Failure(fn, "Label not specified");
        try {
    ...
    if (!self["C3MobileAdvertsAPI"])
        self["C3MobileAdvertsAPI"] = {};
...
;'use strict';
{
    ...
    addHandler("Configure", async (id, pubID, privacyPolicy, showFree, showConsent, debug, debugLocation, showOnStartUp, fn) => {
        if (api["webAdsScriptLoaded"])
            ...
            C3AdUtils.Failure(fn, "advert script not loaded, likely blocked by an ad blocker")
    }
    );
    addHandler("RequestConsent", async fn => {
        C3AdUtils.Success(fn, `UNKNOWN&&not-determined&&true`)
...
;'use strict';
{
    ...
    const HANDLER_CLASS = class MobileAdvertHandler extends self.DOMHandler {
        constructor(iRuntime) {
            ...
            this.AddRuntimeMessageHandlers([actionHandler("CreateBannerAdvert"), actionHandler("ShowBannerAdvert"), actionHandler("HideBannerAdvert"), actionHandler("CreateInterstitialAdvert"), actionHandler("ShowInterstitialAdvert"), actionHandler("CreateVideoAdvert"), actionHandler("ShowVideoAdvert"), actionHandler("CreateRewardedAdvert"), actionHandler("ShowRewardedAdvert"), actionHandler("CreateRewardedInterstitialAdvert"), actionHandler("ShowRewardedInterstitialAdvert"), actionHandler("Configure"), actionHandler("RequestConsent"), actionHandler("SetUserPersonalisation"), actionHandler("SetMaxAdContentRating"), actionHandler("TagForChildDirectedTreatment"), actionHandler("TagForUnderAgeOfConsent"), actionHandler("RequestIDFA"), actionHandler("StatusUpdate"), addScriptHandler("AddScript")])
        }
        _GetApi() {
            if (self["cordova"])
                return self["C3MobileAdvertsAPI"]["real"];
            else if (!self["cordova"] && !USE_EMULATOR)
                return self["C3MobileAdvertsAPI"]["web"];
            else if (USE_EMULATOR)
                return self["C3MobileAdvertsAPI"]["fake"]
        }
        _SuspendRuntime() {
